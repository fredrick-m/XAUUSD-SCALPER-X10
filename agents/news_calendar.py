...
USD_HIGH_IMPACT_KEYWORDS = [
    "Non-Farm Employment Change", "Nonfarm Payrolls", "NFP", "FOMC",
    "Federal Funds Rate", "CPI", "Consumer Price Index", "PPI",
    "Producer Price Index", "GDP", "Gross Domestic Product", "Retail Sales",
    "Unemployment Claims", "Initial Jobless Claims", "Unemployment Rate",
    "Core CPI", "Core PPI", "Core PCE", "PCE Price Index",
    "ISM Manufacturing", "ISM Services", "ADP Non-Farm", "JOLTS",
    "Jackson Hole", "Fed Chair", "Powell",
]
...
def _parse_ff_datetime(date_str: str, time_str: str) -> Optional[datetime]:
    """Parse supported calendar date/time forms into UTC.

    The upstream feed has used both separate date/time fields and ISO-like date
    values over time. Unsupported/tentative times return None rather than being
    guessed into a tradable timestamp.
    """
    if not date_str:
        return None
    ...
    # ISO datetime in the date field.
    try:
        iso = date_str.replace("Z", "+00:00")
        dt = datetime.fromisoformat(iso)
        if dt.hour or dt.minute or "T" in date_str:
...
def _build_static_fallback_events() -> List[dict]:
    """Approximate events for visibility only; never considered verified."""
    ...
    if abs((nfp_date - now).days) <= 7:
        events.append({
            "title": "Non-Farm Employment Change (estimated)",
            "country": "USD", "date": nfp_date.strftime("%m-%d-%Y"),
            "time": "8:30am", "impact": "High", "forecast": "",
            "previous": "", "dt": nfp_date,
        })

    cpi_date = now.replace(day=13, hour=12, minute=30, second=0, microsecond=0)
    if abs((cpi_date - now).days) <= 7:
        events.append({
            "title": "CPI m/m (estimated)", "country": "USD",
            "date": cpi_date.strftime("%m-%d-%Y"), "time": "8:30am",
            "impact": "High", "forecast": "", "previous": "", "dt": cpi_date,
        })
    ...
    if now.month in fomc_months:
        ...
        if abs((fomc_date - now).days) <= 7:
            events.append({
                "title": "FOMC Statement (estimated)", "country": "USD",
                "date": fomc_date.strftime("%m-%d-%Y"), "time": "2:00pm",
                "impact": "High", "forecast": "", "previous": "", "dt": fomc_date,
            })
    return events
...
def is_blackout_period(db, timestamp: Optional[datetime] = None) -> bool:
    ...
    for window in config.get("blackout_windows", []):
        try:
...
class NewsCalendarAgent(BaseAgent):
    ...
    def tick(self):
        ...
        verified = self._last_fetch_source == "forex_factory"

        self.logger.info(
            f"Fetched {len(events)} events, {len(high_impact)} high-impact USD "
            f"source={self._last_fetch_source}"
        )
        ...
        self.set_config("calendar_source", self._last_fetch_source)
        self.set_config("calendar_verified", verified)

        if not verified:
            self.emit_event(
                "warning",
                "Economic calendar is UNVERIFIED; new XAUUSD entries remain blocked until the live feed recovers.",
                {"calendar_source": self._last_fetch_source},
            )
        ...
        if upcoming_7d:
            ...
            for e in upcoming_7d:
                time_str = e["dt"].strftime("%Y-%m-%d %H:%M UTC")
            ...
            self.emit_event(
                ...
                "Upcoming high-impact events (next 7 days):\n" + "\n".join(summary_lines),
                {
                    ...
                    "verified": verified,
                    "events": [
                        {"title": e["title"], "time": e["dt"].isoformat()}
                        for e in upcoming_7d
                    ],
                },
            )
    ...
    def _fetch_events(self) -> List[dict]:
        try:
            ...
            if not isinstance(raw_events, list) or not raw_events:
                raise ValueError("calendar response is empty or not a list")
            ...
            if not events:
                raise ValueError("calendar response contained no usable events")
            self._last_fetch_source = "forex_factory"
            self.logger.info(f"Fetched {len(events)} events from Forex Factory")
            return events
        except Exception as e:
            self._last_fetch_source = "static_fallback"
            self.logger.warning(f"Failed to fetch FF calendar: {e}. Using unverified fallback.")
            self.emit_event(
                "warning",
                f"Calendar API failed: {e}. Using UNVERIFIED static fallback; trading will remain blocked.",
            )
    ...
    def _store_events(self, events: List[dict]):
        for ev in events:
            if not ev.get("dt"):
                continue
            metadata = {
                "title": ev["title"], "country": ev.get("country", "USD"),
                "impact": ev.get("impact", "High"), "event_time": ev["dt"].isoformat(),
