"""
Strategy B003: EMA_5_13_ADX_MACD_EMA50
Family  : EMA_5_13
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S002
Filters : ADX > 25, MACD histogram positive (long) / negative (short), Close > EMA50 (long) / Close < EMA50 (short)
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    long_conf  = trend_filter & (df["MACD_hist"] > 0) & (df["Close"] > df["EMA50"])
    short_conf = trend_filter & (df["MACD_hist"] < 0) & (df["Close"] < df["EMA50"])
...
if __name__ == "__main__":
    ...
    print(f"Strategy B003: EMA_5_13_ADX_MACD_EMA50")
