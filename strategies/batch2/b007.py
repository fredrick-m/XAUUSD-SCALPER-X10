"""
Strategy B007: EMA_5_13_ADX_BB_VOL
Family  : EMA_5_13
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S002
Filters : ADX > 25, Price above BB midline (long) / below BB midline (short), Volume > vol_ma
...
def add_indicators(df: pd.DataFrame, p: dict) -> pd.DataFrame:
    ...
    df["ADX"] = adx_ind.adx()
    bb = ta.volatility.BollingerBands(df["Close"], window=p["bb_period"])
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    long_conf  = trend_filter & (df["Close"] > df["BB_mid"]) & vol_filter
    short_conf = trend_filter & (df["Close"] < df["BB_mid"]) & vol_filter
...
if __name__ == "__main__":
    ...
    print(f"Strategy B007: EMA_5_13_ADX_BB_VOL")
