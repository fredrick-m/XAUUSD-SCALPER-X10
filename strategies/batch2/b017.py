"""
Strategy B017: EMA_5_13_ADX_3BAR_SWING_RSI
Family  : EMA_5_13
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S002
Filters : ADX > 25, 3-bar swing (prev 2 closes going up for long / down for short), RSI > 50 (long) / RSI < 50 (short)
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    # 3-bar swing: previous 2 consecutive closes going up/down
    swing_up   = (df["Close"].shift(1) > df["Close"].shift(2)) & (df["Close"].shift(2) > df["Close"].shift(3))
    swing_down = (df["Close"].shift(1) < df["Close"].shift(2)) & (df["Close"].shift(2) < df["Close"].shift(3))

    long_conf  = trend_filter & swing_up   & (df["RSI"] > 50)
    short_conf = trend_filter & swing_down & (df["RSI"] < 50)
...
if __name__ == "__main__":
    ...
    print(f"Strategy B017: EMA_5_13_ADX_3BAR_SWING_RSI")
