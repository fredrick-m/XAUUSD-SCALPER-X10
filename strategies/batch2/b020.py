"""
Strategy B020: EMA_5_13_ADX_WILLR_EMA_TREND
Family  : EMA_5_13
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S002
Filters : ADX > 25, Williams %R not oversold/overbought, EMA50 trend filter
...
def add_indicators(df: pd.DataFrame, p: dict) -> pd.DataFrame:
    ...
    df["ADX"] = adx_ind.adx()
    df["WILLR"] = ta.momentum.WilliamsRIndicator(df["High"], df["Low"], df["Close"],
                                                  lbp=p["willr_period"]).williams_r()
    return df
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    # Williams %R: not overbought for long (< -20), not oversold for short (> -80)
    willr_not_ob = df["WILLR"] < p["willr_overbought"]  # Williams %R < -20 means not overbought
    willr_not_os = df["WILLR"] > p["willr_oversold"]    # Williams %R > -80 means not oversold

    long_conf  = trend_filter & willr_not_ob & (df["Close"] > df["EMA_trend"])
    short_conf = trend_filter & willr_not_os & (df["Close"] < df["EMA_trend"])
...
if __name__ == "__main__":
    ...
    print(f"Strategy B020: EMA_5_13_ADX_WILLR_EMA_TREND")
