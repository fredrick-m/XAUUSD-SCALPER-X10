"""
Strategy B047: EMA_5_13_ADX_TRIPLE_EMA_ALIGN_RSI
Family  : EMA_NEW_VARIANTS
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S002
Filters : ADX > 25, triple EMA alignment (EMA5 > EMA13 > EMA50 for long), RSI > 50 (long) / RSI < 50 (short)
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    triple_bull = (df["EMA_fast"] > df["EMA_slow"]) & (df["EMA_slow"] > df["EMA50"])
    triple_bear = (df["EMA_fast"] < df["EMA_slow"]) & (df["EMA_slow"] < df["EMA50"])

    long_conf  = trend_filter & triple_bull & (df["RSI"] > 50)
    short_conf = trend_filter & triple_bear & (df["RSI"] < 50)
...
if __name__ == "__main__":
    ...
    print(f"Strategy B047: EMA_5_13_ADX_TRIPLE_EMA_ALIGN_RSI")
