"""
Strategy B049: EMA_5_13_ADX_RSI_DIV_VOL_EMA200
Family  : EMA_NEW_VARIANTS
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S002
Filters : ADX > 25, RSI divergence check (RSI higher while price dipped then recovered), Volume > vol_ma, EMA200 bias
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    n = p["div_lookback"]
    # Bullish divergence proxy: RSI currently higher than n bars ago while price is similar or lower
    rsi_rising   = df["RSI"] > df["RSI"].shift(n)
    price_rising = df["Close"] > df["Close"].shift(n)
    # Bearish divergence proxy: RSI currently lower while price is higher
    rsi_falling   = df["RSI"] < df["RSI"].shift(n)
    price_falling = df["Close"] < df["Close"].shift(n)

    long_conf  = trend_filter & vol_filter & (df["Close"] > df["EMA200"]) & rsi_rising  & price_rising
    short_conf = trend_filter & vol_filter & (df["Close"] < df["EMA200"]) & rsi_falling & price_falling
...
if __name__ == "__main__":
    ...
    print(f"Strategy B049: EMA_5_13_ADX_RSI_DIV_VOL_EMA200")
