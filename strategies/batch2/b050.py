"""
Strategy B050: EMA_9_18_ADX_FULLSTACK_RSI_MACD_VOL_EMA200
Family  : EMA_NEW_VARIANTS
Goal    : XAUUSD-SCALPER-X10 — x10 returns in < 20 days
Timeframe: M1 (XAUUSD)
Base    : S009
Filters : ADX > 25, RSI > 55 / RSI < 45, MACD positive/negative, Volume > vol_ma, EMA200 trend (full stack)
...
def generate_signals(df: pd.DataFrame, p: dict = PARAMS) -> pd.DataFrame:
    ...
    long_conf  = (trend_filter
                  & (df["RSI"] > p["rsi_long"])
                  & (df["MACD_hist"] > 0)
                  & vol_filter
                  & (df["Close"] > df["EMA200"]))

    short_conf = (trend_filter
                  & (df["RSI"] < p["rsi_short"])
                  & (df["MACD_hist"] < 0)
                  & vol_filter
                  & (df["Close"] < df["EMA200"]))
...
if __name__ == "__main__":
    ...
    print(f"Strategy B050: EMA_9_18_ADX_FULLSTACK_RSI_MACD_VOL_EMA200")
